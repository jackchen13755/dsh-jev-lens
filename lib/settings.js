...
export const LENS_MODES = ['off', 'shadow', 'warn', 'gate'];
...
const BOUNDS = {
    ...
    screenTimeoutMs: [200, 30000],
    gateTimeoutMs: [200, 60000],
    sessionCallLimit: [0, 100000],
    dailyCallLimit: [0, 1000000],
};
/**
...
export function validateSettings(value) {
    ...
    if (!LENS_MODES.includes(value.mode))
        return `mode 必须是 ${LENS_MODES.join(' / ')} 之一`;
...
export function mergeSettings(base, patch) {
    ...
    if (typeof p.mode === 'string' && LENS_MODES.includes(p.mode))
        out.mode = p.mode;
    if (typeof p.apiKeyRef === 'string' && p.apiKeyRef.trim())
        out.apiKeyRef = p.apiKeyRef.trim();
    if (typeof p.judgeCommands === 'boolean')
        out.judgeCommands = p.judgeCommands;
    if (typeof p.batchedQuestions === 'boolean')
        out.batchedQuestions = p.batchedQuestions;
    if (typeof p.gateAskOnly === 'boolean')
        out.gateAskOnly = p.gateAskOnly;
...
const settingsFile = (dir) => path.join(dir, 'config.json');
const secretFile = (dir) => path.join(dir, 'secret.json');
/** Read the stored settings. A corrupt or absent file is simply "no opinion". */
export function loadStored(dir) {
    try {
        const parsed = JSON.parse(fs.readFileSync(settingsFile(dir), 'utf8'));
...
export function loadSecret(dir) {
    try {
        const parsed = JSON.parse(fs.readFileSync(secretFile(dir), 'utf8'));
...
export function saveSecret(dir, apiKey) {
    try {
        fs.mkdirSync(dir, { recursive: true });
