 ...
 */
export function resolveGateAction(band, restorable, policy = DEFAULT_GATE_POLICY, approval = undefined) {
    const action = gateAction(band, restorable, policy);
    if (action === 'ask' && approval === 'never')
        return { action: 'allow', degraded: 'gate:ask-unavailable' };
    return { action };
}
/**
