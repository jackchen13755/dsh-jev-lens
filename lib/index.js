...
import { createJev, redact, JevError } from './jev.js';
import { DESTRUCTIVE_KEY, INJECTION_QUESTION, RESTORABLE_KEY, DEFAULT_GATE_POLICY, commandQuestions, destructiveBand, destructiveState, gateAction, gateMessage, } from './questions.js';
...
export function apply(ctx, input = {}) {
    ...
    function authBlocked() {
        ...
        if (Date.now() - auth.at >= config.authCooldownMs) {
            auth.fingerprint = '';
            return false;
        }
     ...
     */
    function degradedRow(where, why, exec) {
        try {
            append(ledgerDir, {
                t: Date.now(),
                kind: 'degraded',
    ...
    async function verdictFor(exec, opts) {
        ...
        if (authBlocked()) {
            degradedRow('command', `auth:rejected-${auth.status}`, exec);
            return null;
        }
