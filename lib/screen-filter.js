...
const PATTERNS = [
    ...
    },
    {
        ...
        test: (text) => /[A-Za-z0-9+/]{180,}={0,2}/.test(text) || /[0-9a-fA-F]{240,}/.test(text),
    },
    {
        name: 'hidden-html',
 ...
 * not a veto.
 */
function unquotedMatch(text, pattern) {
 ...
 */
export function scoreScreen(text) {
    const features = [];
    for (const pattern of PATTERNS) {
        ...
        if (hit)
            features.push({ name: pattern.name, why: pattern.why });
    }
...
export const featureLabel = (verdict) => verdict.features.map(feature => feature.name).join(',');
//# sourceMappingURL=screen-filter.js.map