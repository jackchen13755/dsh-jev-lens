 ...
 */
export function locallySafe(raw) {
    ...
    if (!verb || !SAFE_VERBS.has(verb))
        return null;
    for (const exception of SAFE_VERB_EXCEPTIONS) {
        if (exception.verb === verb && exception.danger.test(command))
            return null;
    }
    if (verb === 'git') {
        // `git status`, `git log` … but a bare `git` can do anything.
        const sub = command.split(/\s+/)[1] ?? '';
        if (!['status', 'log', 'diff', 'show', 'branch', 'remote', 'rev-parse', 'describe', 'blame', 'ls-files', 'config', 'rev-list', 'shortlog', 'tag', 'show-ref', 'for-each-ref'].includes(sub))
            return null;
    }
    if (verb === 'echo' && command.length > 200)
        return null;
...
export function matchDestructive(raw) {
    const command = raw.trim();
    if (!command)
        return null;
...
export function prefilter(raw) {
    const flagged = matchDestructive(raw);
    if (flagged)
        return { kind: 'flag', ...flagged };
    const safe = locallySafe(raw);
    if (safe)
        return { kind: 'skip', reason: safe.reason };
    return { kind: 'judge' };
}
