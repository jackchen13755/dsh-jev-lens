...
/** Per-day file name. */
export function ledgerFile(dir, when = new Date()) {
...
export function summarize(records, days, usdPerMTok = 0.042) {
    const commands = records.filter((r) => r.kind === 'command');
    const outcomes = new Map();
    for (const r of records) {
        if (r.kind === 'command-outcome')
            outcomes.set(`${r.session}:${r.callId}`, r.isError);
    }
    ...
    for (const c of commands)
        if (c.decision)
            gates[c.decision]++;
    for (const r of records) {
        if (r.kind !== 'command-rule' || !r.decision_taken)
            continue;
        gates[r.decision_taken]++;
    }
    const screens = records.filter((r) => r.kind === 'screen');
    const starts = records.filter((r) => r.kind === 'drill-start');
    const ends = new Map();
    for (const r of records)
        if (r.kind === 'drill-end')
            ends.set(r.drillId, r);
    const arm = (name) => {
    ...
    const allTrials = records.filter((r) => r.kind === 'trial');
    ...
    return {
        ...
        screens: {
            ...
            features: (() => {
                ...
                for (const screen of screens) {
                    for (const feature of String(screen.features ?? '').split(',').filter(Boolean)) {
        ...
        coverage: (() => {
            ...
            const skips = records.filter((r) => r.kind === 'degraded');
        ...
        triage: (() => {
            const triaged = records.filter((r) => r.kind === 'triage');
            const declined = records.filter(r => r.kind === 'degraded' && typeof r.reason === 'string' && r.reason.startsWith('triage:')).length;
            return {
                judged: triaged.length,
                flagged: triaged.filter(r => r.level === 'flag' || r.level === 'warn').length,
                cached: records.filter(r => r.kind === 'degraded' && r.reason === 'triage:cached').length,
                declined,
...
export function render(report) {
    ...
    const min = (ms) => (ms >= 1000 ? `${(ms / 1000).toFixed(1)}s` : `${ms}ms`);
    ...
    const lines = [
        ...
        '**B. 注入筛查（对抓取内容打分）**',
        report.screens.n
            ? `筛查 ${report.screens.n} 次 · 其中 ${report.screens.flagged} 次 ≥阈值（${(report.screens.flaggedRate * 100).toFixed(1)}%）· mean p=${report.screens.meanP}${report.screens.cached ? ` · 命中缓存 ${report.screens.cached}` : ''}${report.screens.prefilterSkipped ? ` · **规则前置省下 ${report.screens.prefilterSkipped} 次请求**（无指令特征，不必花钱）` : ''}${report.screens.features.length ? ` · 命中的特征：${report.screens.features.map(f => `${f.feature}×${f.n}`).join(' · ')}` : ''}`
            : '筛查 0 次',
        ...
        ...Object.keys(report.trials).length >= 2
            ? (() => {
                ...
                const lines = [];
                if (b && h)
                    lines.push(`→ harness 规则相对裸页面：${((b.rate - h.rate) * 100).toFixed(0)} 个百分点`);
                if (h && w) {
        ...
        report.coverage.skipped
            ? `⚠️ **判定覆盖 ${(report.coverage.rate * 100).toFixed(0)}%**（判定 ${report.coverage.judged} · 跳过 ${report.coverage.skipped}）${report.coverage.topReasons.length ? ` — 主因：${report.coverage.topReasons.map(r => `${r.reason}×${r.n}`).join(' · ')}` : ''}`
