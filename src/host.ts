 ...
 */
export function serviceOf<T> (ctx: unknown, key: string): T | undefined {
  const anyCtx = ctx as { get?: (name: string) => unknown } & Record<string, unknown>
  let viaGet: unknown
  try {
  ...
  const service = (viaGet ?? direct) as T | undefined
