...
const RUNNERS: Array<{ name: string, pattern: RegExp }> = [
...
function runnerOf (command: string, text: string): string | null {
  for (const runner of RUNNERS) {
    if (runner.pattern.test(command)) return runner.name
  }
  ...
  for (const runner of RUNNERS) {
    if (runner.pattern.test(text.slice(0, 2000))) return runner.name
  }
  return null
}
...
function excerptOf (text: string): string {
  ...
  if (picked.length === 0) return text.slice(0, MAX_EXCERPT)
 ...
 */
export function detectTestFailure (input: { name?: string, command?: string, text?: string, isError?: boolean }): FailureSignal | null {
  ...
  if (!/(bash|shell|exec|command|terminal|run)/i.test(name)) return null
  ...
  if (!runner && !strongSummary) return null
  if (!input.isError && !strongSummary && !/\b\d+\s+failed\b/.test(text)) return null
...
export function renderTriage (signal: FailureSignal, response: { findings?: Array<{ level?: string, headline?: string, values?: Record<string, unknown> }> }): string | null {
