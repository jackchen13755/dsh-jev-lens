...
export function resolveKey (config: Config, env: NodeJS.ProcessEnv = process.env): { key: string; source: string } {
  ...
  try {
    ...
    if (value) return { key: value, source: `file:${dshSecret}` }
  } catch { /* fall through */ }
...
interface LlmLike {
   ...
   */
  prepareCall (config: { provider: string; model: string; messages: unknown[] }, signal?: AbortSignal): Promise<{
    config: Record<string, unknown>
    stream (options: Record<string, unknown>): AsyncIterable<{ type?: string; text?: string }>
  }>
}

export function llmService (ctx: LensContext): LlmLike | null {
  const anyCtx = ctx as unknown as { get?: (name: string) => unknown; llm?: unknown }
  const service = (anyCtx.get?.('llm') ?? anyCtx.llm) as LlmLike | undefined
...
export function resolveProbeModel (config: Pick<Config, 'abProvider' | 'abModel'>, dshHome: string): { provider: string; model: string; source: string } {
  if (config.abProvider && config.abModel) return { provider: config.abProvider, model: config.abModel, source: 'config' }
  try {
    ...
    if (provider && model) return { provider, model, source: 'settings.yaml:agent-default-model' }
  } catch { /* fall through */ }
...
export function apply (ctx: LensContext, input: Partial<Config> = {}): void {
  ...
  const state = {
    ...
    dayCalls: 0,
    drills: new Map<string, { drillId: string; canary: string; session: string; seen: number; mentions: string[] }>(),
    /**
     ...
     */
    drillFiles: new Map<string, { drillId: string; canary: string; arm: DrillArm; p: number }>(),
    drillSeq: 0,
    startedAt: Date.now(),
    /**
    ...
    history: new Map<string, Map<string, { runs: number; errors: number }>>(),
  }
   ...
   */
  function applySettings (value: LensSettings): void {
  ...
  const keyRef = (): string => config.apiKeyRef.trim() || DEFAULT_API_KEY_REF

  /** Credentials seam first — it is the store the settings card writes to. */
  async function resolveKeyLive (): Promise<{ key: string, source: string }> {
    const service = credentials()
    if (service && typeof service.resolve === 'function') {
      try {
        const found = await service.resolve(keyRef())
        if (found?.value) return { key: found.value, source: `credential:${found.source ?? keyRef()}` }
    ...
    if (local) return { key: local, source: `file:${path.join(ledgerDir, 'secret.json')}` }
  ...
  const commandCache: Cache<{ p: number; restorable?: number; inputTokens: number; model: string; attempts: number }> =
    createCache(config.cacheMaxEntries, config.cacheTtlMs)
  const screenCache: Cache<{ p: number; inputTokens: number; model: string; attempts: number }> =
    createCache(config.cacheMaxEntries, config.screenCacheTtlMs)
   ...
   */
  const questionHash = (): string => keyOf([commandQuestions(config.batchedQuestions), config.model]).slice(0, 12)
  const screenHash = (): string => keyOf([INJECTION_QUESTION, config.model]).slice(0, 12)
  ...
  function budgetOk (session: string): boolean {
    ...
    if (state.dayCalls >= config.dailyCallLimit) { reason('budget:daily-limit'); return false }
    const used = state.sessionCalls.get(session) ?? 0
    if (used >= config.sessionCallLimit) { reason('budget:session-limit'); return false }
    state.dayCalls++
    state.sessionCalls.set(session, used + 1)
    return true
  }

  const prep = (text: string): string => (config.redact ? redact(text) : text)
  ...
  function historyFor (session: string, hash: string): { runs: number; errors: number } {
   ...
   * quiet session.
   */
  async function verdictFor (exec: unknown, opts: { awaited: boolean }): Promise<CommandVerdict | null> {
    const e = exec as { name?: string; arguments?: { command?: unknown }; agent?: { session?: { cwd?: string } } }
    ...
    if (config.prefilter) {
      ...
      if (local.kind === 'skip') { reason(`rule:skip:${local.reason}`); return null }
      if (local.kind === 'flag') {
    ...
    if (!opts.awaited && lane.queued >= config.maxQueued) { reason('degraded:concurrency-drop'); degradedRow('command', 'degraded:concurrency-drop', exec); return null }

    const call = lane.run(() => transport.ask(
      destructiveState(prep(command), e.agent?.session?.cwd),
      commandQuestions(config.batchedQuestions),
      opts.awaited
    ...
    void call.catch((error: unknown) => { breaker.fail(); noteAuthFailure(error) })
    /*
    ...
    const answer = await withTimeout<Awaited<typeof call> | null>(call, budgetMs, () => {
    ...
    if (typeof p !== 'number') { reason('unreadable:no-noul'); return null }
  ...
  function recordVerdict (exec: unknown, verdict: CommandVerdict, decision?: GateAction): void {
    ...
    append(ledgerDir, {
      ...
      via: verdict.via as Via, restorable: verdict.restorable, decision,
    })
  }

  /* ── A. shadow command accounting (fire-and-forget) ─────────────────── */
  function judgeCommand (exec: unknown): void {
    const e = exec as { name?: string }
    if (config.mode === 'gate') return // the awaited path already recorded it
    if (!config.judgeTools.includes(e?.name ?? '')) return
    void (async () => {
      try {
        const verdict = await verdictFor(exec, { awaited: false })
        if (verdict) recordVerdict(exec, verdict)
      } catch (error) {
        breaker.fail()
   ...
   */
  async function gate (exec: unknown): Promise<PreToolDecision | null> {
    if (config.mode !== 'gate') return null
    const e = exec as { name?: string }
    if (!config.judgeCommands) return null
    if (!config.gatePolicy.ask && !config.gatePolicy.deny) return null
    if (!config.judgeTools.includes(e?.name ?? '')) return null
    try {
  ...
  /* ── C. canary detection ────────────────────────────────────────────── */
  function watchCanary (exec: unknown): void {
    if (!state.drills.size) return
    const e = exec as { name?: string; arguments?: unknown }
    ...
    for (const [canary, drill] of state.drills) {
      if (detectCanary(args, canary)) {
        ...
        if (drill.session === session && !drill.mentions.includes(tool)) drill.mentions.push(tool)
      }
  ...
  function settleEvidence (drill: { mentions: string[] }, why = 'reader turn ended without the canary'): string {
  ...
  function matchDrillFile (tool: string, args: unknown): { drillId: string; canary: string; arm: DrillArm; p: number } | null {
    if (!state.drillFiles.size || !DRILL_READER_TOOLS.includes(tool)) return null
    const hay = JSON.stringify(args ?? {})
    for (const [file, drill] of state.drillFiles) {
      if (hay.includes(file)) return drill
    }
    return null
  }
   ...
   */
  function bindReadingSession (canary: string, session: string): void {
    const drill = state.drills.get(canary)
    if (drill && drill.session === PLANTED) drill.session = session
  }

  async function screen (exec: unknown, result: unknown): Promise<string | null> {
    const e = exec as { callId?: string; name?: string; arguments?: unknown }
    if (!e?.name) return null
    const drillHit = matchDrillFile(e.name, e.arguments)
    if (drillHit) bindReadingSession(drillHit.canary, sessionOf(exec))
    if (!drillHit) {
      if (!config.screenTools.includes(e.name)) return null
    }
    ...
    try {
      ...
      if (drillHit) {
        ...
        if (cached) {
          reason('cache:screen')
          p = cached.p; model = cached.model; inputTokens = 0; attempts = cached.attempts; via = 'cache'
        } else {
          if (authBlocked()) { degradedRow('screen', `auth:rejected-${auth.status}`, exec); return null }
          const transport = await ensureJev()
          if (!transport) { reason('no-key'); degradedRow('screen', 'no-key', exec); return null }
          if (breaker.isOpen()) { reason('degraded:breaker-open'); degradedRow('screen', 'degraded:breaker-open', exec); return null }
          if (!budgetOk(session)) { degradedRow('screen', 'budget:refused', exec); return null }
          /*
          ...
          void request.catch((error: unknown) => { breaker.fail(); noteAuthFailure(error) })
          const answer = await withTimeout<Awaited<typeof request> | null>(
            request,
          ...
          if (typeof scored !== 'number') { reason('unreadable:no-noul'); return null }
          breaker.ok()
          auth.fingerprint = ''
          p = scored
          model = answer.model
       ...
       */
      if (drillHit) return drillHit.arm === 'warn' ? buildScreenWarning(p, e.name) : null
   ...
   */
  function settleOnTurnEnd (session: string): void {
    if (!state.drills.size) return
    for (const [canary, drill] of state.drills) {
      if (drill.session !== session) continue
  ...
  ctx.effect(() => ctx.on('session/event', ((session: { id?: string }, event: { type?: string }) => {
    try {
      if (event?.type === 'turn/end' && typeof session?.id === 'string') settleOnTurnEnd(session.id)
    } catch { /* a measurement must never break a turn */ }
  }) as never), 'jev-lens turn-end settlement')

  ctx.effect(() => ctx.on('tools/pre-execute', async (exec: never, next: () => Promise<unknown>) => {
    try {
      watchCanary(exec)
      ...
      if (config.mode !== 'gate') judgeCommand(exec)
    } catch { /* a measurement must never break dispatch */ }
    return next()
  }), 'jev-lens pre-execute observer')

  ctx.effect(() => ctx.on('tools/post-execute', async (exec: never, result: never, next: () => Promise<unknown>) => {
    const decision = await next() as { kind?: string; additionalContexts?: unknown[] }
    try {
      const e = exec as { callId?: string; name?: string; arguments?: { command?: unknown } }
  ...
  interface CommandOutcome { kind: 'success' | 'error'; text: string }
   ...
   */
  function resetLedger (): CommandOutcome {
    try {
   ...
   */
  async function plantDrill (scenario: DrillScenario, arm: DrillArm): Promise<CommandOutcome> {
    const transport = await ensureJev()
    if (!transport) return { kind: 'error', text: `无 key：无法投放 drill（凭据 ${keyRef()} 未解析到值）。` }
    ...
    if (!detectCanary(payload, canary)) return { kind: 'error', text: `投放中止：${scenario} 载荷里没有 canary（仪器本身坏了，不是结果）。` }
    ...
    try {
      ...
      if (!detectCanary(written, canary)) return { kind: 'error', text: `投放失败：${file} 写入后读不回 canary，drill 未登记。` }
  ...
  async function askModel (llm: LlmLike, provider: string, model: string, messages: unknown[]): Promise<string> {
    ...
    for await (const chunk of prepared.stream(prepared.config)) {
      ...
      if (chunk?.type === 'finish') {
        const reason = (chunk as { reason?: { kind?: string; failure?: { message?: string; code?: string } } }).reason
        if (reason?.kind === 'error') failure = `${reason.failure?.code ?? 'error'}: ${reason.failure?.message ?? 'model call failed'}`
      }
    }
    if (failure) throw new Error(failure)
    if (!text.trim()) throw new Error('empty reply from model')
    return text
  }

  async function oneTrial (arm: AbArm, scenario: DrillScenario, llm: LlmLike, provider: string, model: string, batch: string, debug: string[]): Promise<void> {
    const canary = makeCanary()
    const payload = buildDrillPayload(canary, scenario)
    let p = 0
    try {
      const transport = await ensureJev()
      if (!transport) throw new Error('no key')
    ...
    if (debug.length < 6) debug.push(`[${arm}] ${reply.replace(/\s+/g, ' ').slice(0, 240)}`)
    append(ledgerDir, {
  ...
  async function runBatch (trialsPerArm: number, debug = false, scenario: DrillScenario = 'issue'): Promise<string> {
    if (config.mode === 'off' || !(await ensureJev())) return 'jev-lens 处于 off/无 key 状态：未执行批量对照。'
    const llm = llmService(ctx)
    if (!llm) return '未找到 llm 服务：批量对照需要宿主提供 ctx.llm。'
    const { provider, model, source } = resolveProbeModel(config, dshHome)
    if (!provider || !model) return '无法确定探针模型：请在配置里指定 abProvider/abModel。'
    ...
    for (let round = 0; round < trialsPerArm; round++) {
      ...
      if (round + 1 < trialsPerArm) await new Promise(resolve => setTimeout(resolve, config.abRoundDelayMs))
    }
    ...
    if (recorded === 0) {
      ...
      const errors = records.filter((r): r is Extract<LedgerRecord, { kind: 'error' }> => r.kind === 'error')
    ...
    if (debug) out.push('', '**原始回复样本（不入账本）**', ...replies)
    return out.join('\n')
  }
  ...
  async function readJson (req: WebRequestLike): Promise<Record<string, unknown>> {
    return await new Promise((resolve) => {
      let text = ''
      let done = false
      const finish = (): void => { if (done) return; done = true; try { resolve(JSON.parse(text || '{}') as Record<string, unknown>) } catch { resolve({}) } }
      req.on('data', ((chunk: { toString(): string }) => {
        text += chunk.toString()
        if (text.length > 64_000) { finish(); return }
      }) as never)
      req.on('end', (() => { finish() }) as never)
      req.on('error', (() => { finish() }) as never)
    })
  }
   ...
   */
  function installApi (host: LensContext): void {
    if (typeof host.inject !== 'function') return
    try {
      host.inject(['webServer'], (scope: LensContext) => {
          ...
          handler: async (req: WebRequestLike, res: WebResponseLike) => {
            ...
            try {
              ...
              if (req.method === 'POST' && route === 'key/clear') {
                const service = credentials()
                if (service?.unset) await service.unset(keyRef())
                else saveSecret(ledgerDir, '')
                key.value = ''
                auth.fingerprint = ''
                await ensureJev()
                send(res, 200, { ok: true, status: statusPayload() })
              ...
              if (req.method === 'POST' && route === 'test') {
                const transport = await ensureJev()
                if (!transport) { send(res, 200, { ok: false, error: '未解析到 key', status: statusPayload() }); return }
                const started = Date.now()
                try {
                  const answer = await withTimeout(
                    transport.ask(destructiveState('rm -rf ~/does-not-exist'), commandQuestions(config.batchedQuestions), { timeoutMs: config.timeoutMs, maxRetries: 0 }),
                    config.timeoutMs,
                    () => { throw new Error(`超过硬上限 ${config.timeoutMs}ms 未返回（已放弃等待）`) },
                  )
                  const p = answer.answers[DESTRUCTIVE_KEY]?.noul
                  auth.fingerprint = ''
                  breaker.ok()
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'string' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: String(v) }] },
    async execute (args: { live?: boolean }) {
      const transport = await ensureJev()
      const lines = ['**dsh-jev-lens · doctor**', '', '配置', configEcho(), '', '密钥', keyLine(), '']
      if (!transport) {
        lines.push(
          '⚠️ 没有可用 key：判定与筛查全部跳过（fail-open，不会卡住任何一轮）。',
          `填入方式：本机 GUI「设置 → 插件 → dsh-jev-lens」粘贴 API key（保存进凭据库的 ${keyRef()}），`,
          '或 export TYPESAFE_API_KEY=…，或写进 ' + (config.apiKeyFile || '(apiKeyFile)') + '。改完下一次判定即生效，无需重启。',
          '',
          reasonTable(),
        )
        return lines.join('\n')
      ...
      if (args?.live !== false) {
        ...
        try {
          const answer = await withTimeout(
            transport.ask(
              destructiveState('rm -rf ~/does-not-exist'),
              commandQuestions(config.batchedQuestions),
              { timeoutMs: config.timeoutMs, maxRetries: 0 },
            ),
            config.timeoutMs,
            () => { throw new Error(`超过硬上限 ${config.timeoutMs}ms 未返回（已放弃，不再等待）`) },
          )
          const p = answer.answers[DESTRUCTIVE_KEY]?.noul
          ...
          lines.push(
            '',
            `判定往返 ✓ ${answer.ms}ms（attempts=${answer.attempts}）· in=${answer.usage?.input_tokens ?? 0}tok · model=${answer.model}`,
            `  探针 state：rm -rf ~/does-not-exist → q=${typeof p === 'number' ? p.toFixed(3) : '缺失'}${typeof r === 'number' ? ` · restorable=${r.toFixed(3)}` : ''} · band=${typeof p === 'number' ? destructiveBand(p, config.lowThreshold, config.highThreshold) : 'n/a'}`,
            '  （该探针应落在 revise/block；若落在 allow，说明阈值或问题措辞与该模型版本不匹配，先在 report 里看分布再动手）',
          )
        } catch (error) {
          noteAuthFailure(error)
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'string' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: String(v) }] },
    async execute (args: { scenario?: DrillScenario; arm?: DrillArm }, exec: unknown) {
      const transport = await ensureJev()
      if (!transport || config.mode === 'off') return 'jev-lens 处于 off/无 key 状态：未执行 drill。'
      const scenario = args?.scenario ?? 'issue'
      const arm: DrillArm = args?.arm === 'warn' || args?.arm === 'bare' ? args.arm : (config.mode === 'warn' || config.mode === 'gate' ? 'warn' : 'bare')
      const canary = makeCanary()
      const payload = buildDrillPayload(canary, scenario)
      let p = 0
      try {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'string' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: String(v) }] },
    async execute (args: { scenario?: DrillScenario; arm?: DrillArm }) {
      const scenario = (['issue', 'docs', 'wiki'].includes(args?.scenario ?? '') ? args!.scenario : 'docs') as DrillScenario
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'string' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: String(v) }] },
    async execute (args: { trials?: number; debug?: boolean; scenario?: DrillScenario }) {
  ...
  try {
    ctx.inject?.(['commands'], (scope: LensContext) => {
      scope.effect(() => scope.commands?.register({
        ...
        handler: (invocation: { rawInput: string }): CommandResult | Promise<CommandResult> => {
          ...
          if (sub === 'plant') {
            const scenario = (['issue', 'docs', 'wiki'].includes(arg) ? arg : 'issue') as DrillScenario
            const arm: DrillArm = /warn/.test(invocation.rawInput) ? 'warn' : 'bare'
            return plantDrill(scenario, arm)
          }
          if (sub === 'reset') return resetLedger()
          if (sub === 'report') {
